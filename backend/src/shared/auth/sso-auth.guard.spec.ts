...
describe('SsoAuthGuard', () => {
  ...
  describe('rejects', () => {
    it('a request with no credential at all', async () => {
      await await expect(guard.canActivate(contextFor({ headers: {} }))).rejects.toThrow(UnauthorizedException);
    });
  ...
  describe('service token (machine callers such as the MCP server)', () => {
    it('accepts the correct service token', async () => {
      ...
      expect(request.user).toEqual(expect.objectContaining({ displayName: 'Martin Hylleberg' }));
