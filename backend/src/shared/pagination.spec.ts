...
describe('normalisePageRequest', () => {
  it('defaults when nothing is supplied', () => {
    expect(normalisePageRequest({})).toEqual({
  ...
  it('honours a sensible request', () => {
    expect(normalisePageRequest({ limit: 20, offset: 40 })).toEqual({
      limit: 20,
      offset: 40,
    });
  });
  ...
  it('survives garbage from the query string', () => {
    ...
    expect(normalisePageRequest({ limit: Infinity }).limit).toBe(
      DEFAULT_PAGE_LIMIT,
    );
  });

  it('floors a fractional page request', () => {
    expect(normalisePageRequest({ limit: 10.9, offset: 5.9 })).toEqual({
      limit: 10,
      offset: 5,
    });
  });
...
describe('toPagedResponse', () => {
  it('reports hasMore when the page does not reach the total', () => {
    const response = toPagedResponse({
      data: [1, 2],
      total: 10,
      limit: 2,
      offset: 0,
  ...
  it('reports hasMore false when everything fits in one page', () => {
    expect(
      toPagedResponse({ data: [1], total: 1, limit: 100, offset: 0 }).meta
        .hasMore,
    ).toBe(false);
  });
  ...
  it('reports hasMore false for an empty result', () => {
    ...
    expect(
      toPagedResponse({ data: [], total: 0, limit: 100, offset: 0 }).meta
        .hasMore,
    ).toBe(false);
  });
  ...
  it('reports hasMore false for an offset past the end', () => {
    expect(
      toPagedResponse({ data: [], total: 5, limit: 10, offset: 50 }).meta
        .hasMore,
    ).toBe(false);
  });
  ...
  it('passes the data through untouched', () => {
    const data = [{ id: 'a' }];
    expect(
      toPagedResponse({ data, total: 1, limit: 100, offset: 0 }).data,
    ).toBe(data);
  });
});
