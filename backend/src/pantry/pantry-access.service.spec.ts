    ...
    pantryMember: {
      findUnique: jest.fn(
        ({
          ...
          where: { pantryId_userId: { pantryId: string; userId: string } };
        }) =>
          Promise.resolve(
            memberships.find(
              (m) =>
...
describe('PantryAccessService.resolve', () => {
  describe('with an explicit pantry id', () => {
    ...
    it('REFUSES a non-member with 403, not an empty result', async () => {
      ...
      await expect(svc.resolve(heidi, 'p-home')).rejects.toThrow(
        ForbiddenException,
      );
    });
    ...
    it('refuses a pantry that does not exist, rather than falling back to the default', async () => {
      ...
      await expect(svc.resolve(martin, 'p-nonexistent')).rejects.toThrow(
        ForbiddenException,
      );
    });
  });
  ...
  describe('isolation between households', () => {
    it('never leaks another household even when both users are known', async () => {
      ...
      await expect(svc.resolve(martin, 'p-heidi')).rejects.toThrow(
        ForbiddenException,
      );
      await expect(svc.resolve(heidi, 'p-martin')).rejects.toThrow(
        ForbiddenException,
      );
    });
  });
